    ...
    if (block != null && block.transactions != null) {
      block.transactions.forEach( function(e) {
        ...
        if (e.to == icoAddress && txOk) {
          ...
          contributingAddresses[e.from] = newTokenBalance;
  ...
  filter.watch(function(e, r) {
    ...
    if (toAddress == icoAddress) {
      ...
      if (token != null) {
        ...
        contributingAddresses[fromAddress] = newTokenBalance;
      }
    }
  });
}

function getNewTokenBalances() {
  var addresses = {};
  console.log("RESULT: Type\tAddress\tBlock\t#\tAmount\tBalance\tICO Contributed");
  var count = 0;
  var filter = web3.eth.filter({fromBlock: tokenFillStartBlock, toBlock: newTokenFillEndBlock, address: newTokenAddress, topics: [["0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef"]]});
  filter.watch(function(e, r) {
    // console.log("RESULT: " + JSON.stringify(r));
    var fromAddress = "0x" + r.topics[1].substring(26, 66);
    var toAddress = "0x" + r.topics[2].substring(26, 66);
    if (fromAddress == "0x0000000000000000000000000000000000000000") {
      ...
      var tokenBalance = newToken.balanceOf(toAddress, newTokenFillEndBlock).div(1e8);
      var icoContributed = (toAddress in contributingAddresses);
      console.log("RESULT: Transfers\t" + toAddress + "\t" + r.blockNumber + "\t" + count + "\t" + amount + "\t" + tokenBalance + "\t" + icoContributed);
      addresses[toAddress] = tokenBalance;
    }
  });

  console.log("RESULT: Type\tAddress\t#\tAmount\tBalance\tICO Contributed");
  count = 0;
  for (var address in addresses) {
    count++;
    var tokenBalance = newToken.balanceOf(address, newTokenFillEndBlock).div(1e8);
    var icoContributed = (address in contributingAddresses);
    console.log("RESULT: Balance\t" + address + "\t" + count + "\t" + tokenBalance + "\t" + icoContributed);
  }
}
